package deq
// aoc/deq is full repo folder path

import "fmt"

///////////////////////////////////////////////////////////
//                                                       //
// Deq is a stack/queue combo (double ended queue)       //
//                                                       //
// https://github.com/gammazero/deque/blob/main/deque.go //
// v1.0.0 Downloaded 2024-12-18                          //
//                                                       //
//   q := new(deq.Deq[int])                              //
//   // or                                               //
//   var q deq.Deq[int]                                  //
//   q.SetBaseCap(1000) // if you know how big it grows  //
//   if q.Len() < 1 { break } // always check size       //
//                                                       //
// Stacklike use:                                        //
//   q.PushBack(17)      i = q.PopBack()                 //
//                                                       //
// Queuelike use:                                        //
//   q.PushBack(17)      i = q.PopFront()                //
//                                                       //
///////////////////////////////////////////////////////////
//                                                       //
// type Deq[T any] struct {...}                          //
//                                                       //
// func (q *Deq[T]) Cap() int                            //
// func (q *Deq[T]) Len() int                            //
// func (q *Deq[T]) PushBack(elem T)                     //
// func (q *Deq[T]) PushFront(elem T)                    //
// func (q *Deq[T]) PopFront() T                         //
// func (q *Deq[T]) PopBack() T                          //
// func (q *Deq[T]) Front() T                            //
// func (q *Deq[T]) Back() T                             //
// func (q *Deq[T]) At(i int) T                          //
// func (q *Deq[T]) Set(i int, item T)                   //
// func (q *Deq[T]) Clear()                              //
// func (q *Deq[T]) Grow(n int)                          //
// func (q *Deq[T]) Rotate(n int)                        //
// func (q *Deq[T]) Index(f func(T) bool) int            //
// func (q *Deq[T]) RIndex(f func(T) bool) int           //
// func (q *Deq[T]) Insert(at int, item T)               //
// func (q *Deq[T]) Remove(at int) T                     //
// func (q *Deq[T]) SetBaseCap(baseCap int)              //
// func (q *Deq[T]) Swap(idxA, idxB int)                 //
//                                                       //
///////////////////////////////////////////////////////////


// minCapacity is the smallest capacity that deq may have. Must be power of 2
// for bitwise modulus: x % n == x & (n - 1).
const minCapacity = 16

// Deq represents a single instance of the deque data structure, similar to
// python3's `collections.deque` class. A Deq instance contains items of the type
// specified by the type argument.
//
// For example, to create a Deq that contains strings do one of the
// following:
//
//	var stringDeq deq.Deq[string]
//	stringDeq := new(deq.Deq[string])
//	stringDeq := &deq.Deq[string]{}
//
// To create a Deq that will never resize to have space for less than 64
// items, specify a base capacity:
//
//	var d deq.Deq[int]
//	d.SetBaseCap(64)
//
// To ensure the Deq can store 1000 items without needing to resize while
// items are added:
//
//	d.Grow(1000)
//
// Any values supplied to SetBaseCap and Grow are rounded up to the nearest
// power of 2, since the Deq grows by powers of 2.
type Deq[T any] struct {
	buf    []T
	head   int
	tail   int
...
// Cap returns the current capacity of the Deq. If q is nil, q.Cap() is zero.
func (q *Deq[T]) Cap() int {
	if q == nil {
		return 0
	}
...
func (q *Deq[T]) PopFront() T {
	if q.count <= 0 {
		panic("deq: PopFront() called on empty queue")
	}
	ret := q.buf[q.head]
	var zero T
	q.buf[q.head] = zero
...
func (q *Deq[T]) PopBack() T {
	if q.count <= 0 {
		panic("deq: PopBack() called on empty queue")
	}
...
func (q *Deq[T]) Front() T {
	if q.count <= 0 {
		panic("deq: Front() called when empty")
	}
	return q.buf[q.head]
}
...
func (q *Deq[T]) Back() T {
	if q.count <= 0 {
		panic("deq: Back() called when empty")
	}
...
// call panics.
//
// The purpose of At is to allow Deq to serve as a more general purpose
// circular buffer, where items are only added to and removed from the ends of
// the deq, but may be read from any place within the deq. Consider the
...
// Set assigns the item to index i in the queue. Set indexes the deq the same
...
// Grow grows deq's capacity, if necessary, to guarantee space for another n
// items. After Grow(n), at least n items can be written to the deq without
// another allocation. If n is negative, Grow panics.
func (q *Deq[T]) Grow(n int) {
	if n < 0 {
		panic("deq.Grow: negative count")
	}
	c := q.Cap()
	l := q.Len()
...
// Rotate rotates the deq n steps front-to-back. If n is negative, rotates
// back-to-front. Having Deq provide Rotate avoids resizing that could happen
...
// Index returns the index into the Deq of the first item satisfying f(item),
...
// pushing the item onto the front of back of the deq.
//
// Important: Deq is optimized for O(1) operations at the ends of the queue,
...
func (q *Deq[T]) checkRange(i int) {
	if i < 0 || i >= q.count {
		panic(fmt.Sprintf("deq: index out of range %d with length %d", i, q.Len()))
	}
}

// prev returns the previous buffer position wrapping around buffer.
func (q *Deq[T]) prev(i int) int {
	return (i - 1) & (len(q.buf) - 1) // bitwise modulus
}

// next returns the next buffer position wrapping around buffer.
func (q *Deq[T]) next(i int) int {
	return (i + 1) & (len(q.buf) - 1) // bitwise modulus
}

// growIfFull resizes up if the buffer is full.
func (q *Deq[T]) growIfFull() {
...
func (q *Deq[T]) shrinkIfExcess() {
	if len(q.buf) > q.minCap && (q.count<<2) == len(q.buf) {
		q.resize(q.count << 1)
	}
}

// resize resizes the deq to fit exactly twice its current contents. This is
// used to grow the queue when it is full, and also to shrink it when it is
// only a quarter full.
func (q *Deq[T]) resize(newSize int) {
